...
class ImagineService(GlobalConfigs):
    ...
    def get_images_wo_upscale(self, prompt:str, foldername: str, idx:int = -1, crop: bool = True, realism: bool = True, close_up: bool = True) -> bool:
        ...
        """
        self.imagine(prompt=prompt, realism=realism, close_up=close_up)
        # Waiting for the response while the imagine command finishes generating images grid
        time.sleep(11)
        try:
            while True:
                self.last_imagine_msg = self.get_last_message()
        ...
        try:
            ...
            folder_name = f"./overall/{foldername.replace(' ', '_').replace('.', '_')}"
            ...
            for i in range(2):
                for j in range(2):
                    ...
                    filename = os.path.join(os.getcwd(), f'{folder_name}', f'{prompt[:16]}_{uuid.uuid4().hex}.jpg')
